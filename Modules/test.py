...
device = get_default_device()
model = SiameseNetwork()
model.load_state_dict(torch.load('../Output/Models/model_2.pth', map_location=torch.device('cuda')))
to_device(model, device)

t = Transforms()

target = r"../Datasets/Raw/suspects/content/drive/Shareddrives/ZINDI Data Science/ADPL/Competition Data/CV/Data Prep/Test (0-1599)/merged/crops/image_0523.png"
...
for result in results.xyxy[0].tolist():
  ...
  print(float(predict_image('../Output/Models/model_2.pth', target, plushie, transform=t))) # > 0 means match, < 0 means no match
